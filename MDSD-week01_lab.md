...
<img width="1920" height="1020" alt="สกรีนช็อต 2026-07-09 174336" src="https://github.com/user-attachments/assets/24712bb8-c9f9-40f5-bce6-e017c6223981" />
...
<img width="1292" height="871" alt="สกรีนช็อต 2026-07-09 190825" src="https://github.com/user-attachments/assets/ff94b784-f006-48d6-bcf0-bd335df7de93" />
...
<img width="1920" height="1020" alt="image" src="https://github.com/user-attachments/assets/d470b76c-8ff8-444f-be92-db79a5c145e7" />
    ...
    └── Scaffold
        ...
        └── Padding 
            └── Column 
                ...
                │       ├── Row (วิชาที่ชอบ)
                │       └── Row (เป้าหมาย)
                └── ElevatedButton.icon (ปุ่มเปิด AI Chat)
```
...
|---|---|---|
| ความเร็ว | เร็วมาก (มักใช้เวลาต่ำกว่า 1 วินาที) เพราะโหลดเฉพาะโค้ดที่เปลี่ยน | ช้ากว่า (ใช้เวลาไม่กี่วินาที) เพราะต้องเคลียร์ระบบและคอมไพล์ใหม่ทั้งหมด |
| State ถูก Reset? | ไม่ถูก Reset (รักษาสถานะเดิมไว้ เช่น พิมพ์ข้อความค้างไว้ หรือค่าตัวแปรในหน้าจอก็ยังอยู่เหมือนเดิม) | ถูก Reset ทั้งหมด (แอปจะกลับไปเริ่มนับหนึ่งใหม่ที่หน้าแรก เหมือนเพิ่งเปิดแอปขึ้นมาใหม่) |
| ใช้เมื่อไหร่ | ใช้ตอนแต่ง UI, เปลี่ยนสี, ปรับฟอนต์, และแก้บั๊กเล็กๆ ในหน้าจอ โดยที่แอปไม่ต้องเริ่มใหม่ | ใช้ตอนแก้โค้ดใน main(), initState(), เปลี่ยนตัวแปร Global, หรือเพิ่มรูปภาพ/Asset ใหม่เข้าโปรเจกต์ |

### 3.4 ผลการทดลอง Prompt Engineering

**Prompt แบบ Simple:**
```
เขียนโค้ดหน้า Profile ใน Flutter ให้หน่อย มีรูปโปรไฟล์แบบวงกลมตรงกลาง ใช้โทนสีส้ม (Orange) ในการตกแต่ง
```
<img width="1920" height="1020" alt="สกรีนช็อต 2026-07-09 202754" src="https://github.com/user-attachments/assets/477198ea-2a10-4eea-a540-8efca43baad2" />

**Prompt แบบ Detailed:**
```
คุณเป็นผู้เชี่ยวชาญด้านการพัฒนา Mobile Application ด้วย Flutter (เวอร์ชันล่าสุด)
จงเขียนโค้ดสำหรับหน้า Profile Page (StatelessWidget) โดยมีรายละเอียดดังต่อไปนี้:
1. ส่วนบน (Header): ใช้ CircleAvatar แสดงรูปโปรไฟล์ตัวอย่าง ตรงกลาง และมีชื่อ-นามสกุล (Text ตัวหนา ขนาด 24) วางอยู่ด้านล่างรูป
2. ส่วนข้อมูล (Content): ใช้ Card ห่อหุ้มรายการข้อมูล โดยข้างในประกอบด้วยแถวข้อมูล (Row) 3 แถว คือ รหัสนักศึกษา,
สถาบัน และ อีเมล โดยให้มีไอคอน (Icon) นำหน้าข้อมูลแต่ละแถวให้เหมาะสม
3. ส่วนท้าย (Footer): มีปุ่ม ElevatedButton.icon เขียนว่า "แก้ไขข้อมูล" วางไว้ล่างสุดของหน้าจอ
4. ธีมสี: ใช้โทนสีส้ม (Colors.orange) เป็นสีหลักของหน้าจอ (AppBar และ ปุ่ม)
5. ข้อกำหนดโค้ด: เขียนโค้ดให้สะอาด (Clean Code), ใส่ const ในจุดที่จำเป็น และไม่มีการเรียกใช้ Library ภายนอกนอกจาก
package:flutter/material.dart
```
<img width="1920" height="1020" alt="สกรีนช็อต 2026-07-09 203024" src="https://github.com/user-attachments/assets/37e8c0d9-3d0b-4df4-8ec4-e59c24941412" />

**ความแตกต่างของผลลัพธ์:**
```
Prompt แบบ Simple Gemini API สามารถประมวลผลและตอบกลับมาเป็นโค้ดโปรแกรม Flutter ได้สำเร็จและรวดเร็วทันที
เนื่องจากตัวคำสั่งมีความสั้นและซับซ้อนน้อย ตัว Model จึงใช้เวลาและพลังงานในการประมวลผล (Compute Token) ต่ำ

Prompt แบบ Detailed ตัวแอปพลิเคชันฝั่ง Client ได้รับข้อความตอบกลับเป็น Error 503 (This model is currently experiencing high demand...)
ซึ่งเกิดจากตัว Prompt มีความยาว รายละเอียด และเงื่อนไขข้อกำหนดที่เยอะมาก (Complex Prompt) ประกอบกับในช่วงเวลานั้นเซิร์ฟเวอร์ของ Gemini มีผู้ใช้งานหนาแน่น
ส่งผลให้ระบบประมวลผลไม่ทันหรือขาดการเชื่อมต่อ (Timeout) ระหว่างพ่นคำตอบออกมา
```

### 3.5 Screenshot ของ AI Chat App

<img width="1920" height="1020" alt="image" src="https://github.com/user-attachments/assets/376e1367-aecd-4bd7-9d74-d1814c4b27cf" />
...
```
คำตอบ: Flutter ใช้ Rendering Engine ของตัวเอง (ชื่อ Impeller หรือ Skia) ในการวาดและควบคุมพิกเซลทุกจุดบนหน้าจอโดยตรง
ทำให้ได้ UI ที่สม่ำเสมอและลื่นไหลเท่ากันทุกแพลตฟอร์ม ส่วน React Native จะไม่ได้วาดเอง แต่จะแปลงโค้ดจาวาสคริปต์ผ่าน Bridge
เพื่อไปเรียกใช้ Native Component (UI ดั้งเดิม) ของ iOS และ Android ในการแสดงผลแทน
```

**2.** อธิบายความแตกต่างระหว่าง `StatelessWidget` และ `StatefulWidget` พร้อมยกตัวอย่างการใช้งานที่เหมาะสมของแต่ละประเภท

```
คำตอบ: StatelessWidget: เป็น Widget ประเภทคงที่ เปลี่ยนแปลงค่าไม่ได้หลังจากถูกสร้างขึ้น (ไม่มีการเก็บ State)
เหมาะกับ UI ที่แสดงผลอย่างเดียว เช่น หน้าข้อความนิ่งๆ (About Page), โลโก้, หรือรูปโปรไฟล์

StatefulWidget: เป็น Widget ประเภทไดนามิก สามารถเปลี่ยนแปลงหน้าตาหรือข้อมูลภายในได้แบบเรียลไทม์เมื่อผู้ใช้มีปฏิสัมพันธ์
(เช่น การสั่ง setState()) เหมาะกับ UI ที่มีการอัปเดตข้อมูล เช่น หน้าจอแชต, ตะกร้าสินค้า, ปุ่มกดนับตัวเลข, หรือหน้ากรอกฟอร์ม
```

**3.** เหตุใดจึงห้าม Commit API Key ลง Git Repository? และมีวิธีจัดการ API Key อย่างปลอดภัยอย่างไรบ้าง?

```
คำตอบ: เพราะ API Key เปรียบเสมือนรหัสผ่านเข้าบ้าน หากหลุดเข้าไปใน Git Repository (โดยเฉพาะ Public บน GitHub)
จะเสี่ยงต่อการโดนแฮกเกอร์ขโมยไปใช้งาน แอบอ้างสิทธิ์ หรือนำไปรันจนเกิดค่าใช้จ่ายมหาศาล

ธีจัดการอย่างปลอดภัย ให้เก็บ API Key ไว้ในไฟล์ภายนอก เช่น ไฟล์ .env แล้วเพิ่มชื่อไฟล์นี้ลงใน .gitignore
เพื่อไม่ให้ถูกอัปโหลดขึ้น Git จากนั้นใช้แพ็กเกจอย่าง flutter_dotenv มาดึงค่าไปใช้งานในแอป
```

**4.** Hot Reload ทำงานอย่างไร และมีข้อจำกัดอะไรบ้าง?

```
คำตอบ: Hot Reload ทำงานโดยการฉีดโค้ด (Source Code) ส่วนที่มีการแก้ไขเข้าไปยัง Dart Virtual Machine (VM)
ที่กำลังรันอยู่ ทำให้ UI อัปเดตทันทีโดยที่ยังคงสถานะ (State) เดิมของแอปไว้

ข้อจำกัด: ไม่สามารถอัปเดตการแก้ไขที่เกิดขึ้นในฟังก์ชัน main(), โค้ดในส่วนการตั้งค่าเริ่มต้นอย่าง initState(),
การเปลี่ยนโครงสร้างตัวแปร Global/Static และการเพิ่ม/เปลี่ยนไฟล์ Asset (รูปภาพ, ฟอนต์) ใหม่ๆ เข้ามา
ซึ่งเคสเหล่านี้ต้องใช้ Hot Restart แทน
```
...
```
คำตอบ: 
1. AI Chatbot & Virtual Assistant สร้างระบบแชตบอตอัจฉริยะในแอปเพื่อคอยตอบคำถาม ให้คำแนะนำ หรือช่วยเหลือผู้ใช้งานแบบเรียลไทม์
2. Smart Content Generator นำมาช่วยวิเคราะห์ สรุปเนื้อหา หรือสร้างข้อความ/คำอธิบายอัตโนมัติ เช่น แอปสรุปบทเรียน หรือแอปช่วยคิดแคปชัน
3. Code & Bug Assistant นำมาช่วยรีวิวโค้ด Flutter, หาจุดบั๊ก, และแนะนำโครงสร้างโค้ดที่ดีขึ้น (Refactor) ในระหว่างที่นักพัฒนากำลังเขียนแอป
```
