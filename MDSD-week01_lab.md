...
Flutter Version: Version 10.0.26200.8655
Dart Version:  3.12.2
...
```
[<img width="1223" height="921" alt="image" src="https://github.com/user-attachments/assets/44d3d027-cff2-4410-9279-fcd91c663daa" />]
...
|---|---|---|
| ความเร็ว |เร็วกว่า ใช้เวลาไม่กี่วินาที |ช้ากว่า เพราะต้องเริ่มแอปใหม่ |
| State ถูก Reset? |ไม่ถูก Reset ยังคงค่าข้อมูลเดิม | ถูก Reset กลับค่าเริ่มต้น|
| ใช้เมื่อไหร่ |ใช้เมื่อแก้ไข UI, สี, Layout หรือโค้ดบางส่วนที่ไม่กระทบ State |ใช้เมื่อแก้ไขโครงสร้างหลักของแอป หรือเมื่อต้องการเริ่ม State ใหม่ |

### 3.4 ผลการทดลอง Prompt Engineering

**Prompt แบบ Simple:**
```
(สร้างแอป Flutter สำหรับแชทกับ AI)
```

**Prompt แบบ Detailed:**
```
(สร้างแอป Flutter สำหรับแชทกับ Gemini AI API 
โดยมีหน้าจอ Chat Interface มีช่องกรอกข้อความ ปุ่มส่งข้อความ 
แสดงประวัติการสนทนา รองรับ Loading ระหว่างรอคำตอบ 
ใช้ Material Design และเขียนโค้ดที่สามารถนำไปใช้งานจริงได้)
```

**ความแตกต่างของผลลัพธ์:**
```
(Prompt แบบ Simple ได้ผลลัพธ์ที่เป็นแนวทางทั่วไป
และมีรายละเอียดของโค้ดน้อยกว่า

Prompt แบบ Detailed ได้ผลลัพธ์ที่ตรงตามความต้องการมากกว่า
มีโครงสร้างแอป ฟังก์ชัน และรายละเอียดการทำงานที่ชัดเจน
ทำให้สามารถนำไปพัฒนาต่อได้ง่ายกว่า)
```
...
```
คำตอบ: คำตอบ: Flutter ใช้ Rendering Engine ของตัวเองชื่อ Skia ในการวาด UI ทั้งหมด
ทำให้สามารถควบคุมการแสดงผลได้โดยตรงและมีประสิทธิภาพสูง
ส่วน React Native ใช้ Native Component ของระบบปฏิบัติการ
เช่น Android และ iOS ในการแสดงผล UI
```

**2.** อธิบายความแตกต่างระหว่าง `StatelessWidget` และ `StatefulWidget` พร้อมยกตัวอย่างการใช้งานที่เหมาะสมของแต่ละประเภท

```
คำตอบ: คำตอบ: StatelessWidget เป็น Widget ที่ไม่มีการเปลี่ยนแปลงข้อมูลระหว่างการทำงาน
เหมาะสำหรับหน้าจอหรือส่วนประกอบที่แสดงข้อมูลคงที่ เช่น Text หรือ Icon

StatefulWidget เป็น Widget ที่สามารถเปลี่ยนแปลงข้อมูลหรือสถานะได้
เหมาะสำหรับส่วนที่มีการโต้ตอบกับผู้ใช้ เช่น TextField, ปุ่มกด หรือหน้าจอแชท
```

**3.** เหตุใดจึงห้าม Commit API Key ลง Git Repository? และมีวิธีจัดการ API Key อย่างปลอดภัยอย่างไรบ้าง?

```
คำตอบ: คำตอบ: ไม่ควร Commit API Key ลง Git Repository เพราะบุคคลอื่นสามารถเข้าถึง Key
และนำไปใช้งานโดยไม่ได้รับอนุญาต ซึ่งอาจทำให้เกิดค่าใช้จ่ายหรือความเสียหายด้านความปลอดภัย

วิธีจัดการ API Key อย่างปลอดภัย เช่น ใช้ไฟล์ .env,
Environment Variables, Secret Manager และกำหนดสิทธิ์การเข้าถึง Key ให้เหมาะสม
```

**4.** Hot Reload ทำงานอย่างไร และมีข้อจำกัดอะไรบ้าง?

```
คำตอบ: คำตอบ: Hot Reload เป็นความสามารถของ Flutter ที่ช่วยอัปเดตโค้ดที่แก้ไข
โดยไม่ต้องปิดและเปิดแอปใหม่ ทำให้นักพัฒนาสามารถเห็นผลลัพธ์ได้อย่างรวดเร็ว

ข้อจำกัดคือไม่สามารถใช้ได้กับการเปลี่ยนแปลงบางส่วนของโครงสร้างแอป
เช่น การเปลี่ยนแปลงใน main() หรือการแก้ไข State บางประเภท
ซึ่งอาจจำเป็นต้องใช้ Hot Restart แทน
```
...
```
คคำตอบ:
1. ช่วยสร้างและแนะนำโค้ด เช่น การสร้าง Widget หรือฟังก์ชันต่าง ๆ ใน Flutter

2. ช่วยตรวจสอบและแก้ไขข้อผิดพลาดของโค้ด (Debugging)

3. ช่วยสร้างฟีเจอร์ AI ภายในแอป เช่น ระบบ Chatbot, ระบบตอบคำถามอัตโนมัติ หรือการวิเคราะห์ข้อมูล
```
