...
```
คำตอบ: Flutter มี Rendering Engine ของตัวเอง จึงวาดหน้าจอทั้งหมดเอง ทำให้ UI ลื่นไหลและมีความสม่ำเสมอในทุกแพลตฟอร์ม
React Native ใช้ Rendering Engine ของระบบปฏิบัติการ โดยแปลง Component ไปเป็น Native UI ของ Android และ iOS ทำให้ได้หน้าตาแบบ Native แต่การแสดงผลอาจแตกต่างกันระหว่างแต่ละแพลตฟอร์ม
```

**2.** อธิบายความแตกต่างระหว่าง `StatelessWidget` และ `StatefulWidget` พร้อมยกตัวอย่างการใช้งานที่เหมาะสมของแต่ละประเภท

```
คำตอบ: StatelessWidget ใช้สำหรับ UI ที่ข้อมูลคงที่ ไม่เปลี่ยนแปลงหลังจากสร้าง Widget เช่น โลโก้ ข้อความ หรือหน้าข้อมูลทั่วไป
StatefulWidget ใช้สำหรับ UI ที่ข้อมูลสามารถเปลี่ยนแปลงได้และต้องอัปเดตการแสดงผล เช่น ตัวนับจำนวน ฟอร์ม หรือปุ่มที่มีการโต้ตอบกับผู้ใช้ โดยใช้ setState() เพื่อสั่งให้หน้าจอแสดงผลใหม่ตามสถานะล่าสุด
```

**3.** เหตุใดจึงห้าม Commit API Key ลง Git Repository? และมีวิธีจัดการ API Key อย่างปลอดภัยอย่างไรบ้าง?

```
คำตอบ: ห้าม Commit API Key ลง Git Repository เพราะเสี่ยงต่อการรั่วไหล ทำให้ผู้อื่นนำไปใช้งานโดยไม่ได้รับอนุญาต ส่งผลให้ข้อมูลไม่ปลอดภัยและอาจเกิดค่าใช้จ่ายเพิ่ม
```

**4.** Hot Reload ทำงานอย่างไร และมีข้อจำกัดอะไรบ้าง?

```
คำตอบ: **Hot Reload** คือการอัปเดตโค้ดที่แก้ไขแล้วไปยังแอปที่กำลังรันอยู่ทันที โดยไม่ต้องปิดและเปิดแอปใหม่ ทำให้เห็นผลลัพธ์ได้รวดเร็วและยังคงสถานะ (State) ของแอปไว้

**ข้อจำกัด:** ใช้ไม่ได้กับการเปลี่ยนแปลงบางอย่าง เช่น การแก้ `main()` หรือการเปลี่ยนโครงสร้างเริ่มต้นของแอป ซึ่งกรณีเหล่านี้ต้องใช้ **Hot Restart** หรือรันแอปใหม่ (Full Restart) แทน
```
...
```
คำตอบ: 
1. Chatbot ตอบคำถามอัตโนมัติ – ใช้ AI ตอบคำถามผู้ใช้ ให้ข้อมูลสินค้า หรือช่วยแก้ปัญหาเบื้องต้นได้ตลอดเวลา
2. สรุปและวิเคราะห์ข้อมูล – ใช้ AI สรุปข้อความหรือเอกสารยาว ๆ และดึงข้อมูลสำคัญออกมาเพื่อให้เข้าใจได้ง่ายขึ้น
3. สร้างคอนเทนต์อัตโนมัติ – ใช้ AI ช่วยเขียนคำอธิบายสินค้า โพสต์บนโซเชียล หรือร่างอีเมลจากข้อมูลที่ผู้ใช้ให้มา
```
