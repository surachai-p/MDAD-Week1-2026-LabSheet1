...
└── สลับหน้าจอด้วย Navigator (ขึ้นอยู่กับว่าผู้ใช้อยู่ที่หน้าไหน)
    ...
    │   ├── AppBar (แถบหัวเรื่อง Profile)
    │   └── SingleChildScrollView (พื้นที่เลื่อนแนวตั้ง)
    │       └── Center
    │           └── Column (จัดเรียงข้อมูลส่วนตัวลงมา)
    │               ├── CircleAvatar (รูปโปรไฟล์)
    │               ├── Text ('Kittikarn Kaewnual')
    │               ├── Card (ข้อมูล คณะ / วิชาที่ชอบ / คติ)
    │               └── ElevatedButton.icon (ปุ่มสำหรับกดไปหน้า AI Chat)
    │
    └── หน้าที่ 2: AiChatPage (Scaffold)  <-- เมื่อกดปุ่มจะสลับมาต้นไม้ฝั่งนี้
        ├── AppBar (แถบหัวเรื่อง AI Chat)
...
|---|---|---|
| ความเร็ว |เร็วมาก ใช้เวลาไม่ถึง 1 วินาที|เร็วกว่ารันใหม่ แต่ช้ากว่า Hot Reload เล็กน้อย |
| State ถูก Reset? |ไม่รีเซ็ต ข้อมูลเดิมบนหน้าจอยังคงอยู่ครบ |รีเซ็ตข้อมูลบนหน้าจอทั้งหมด กลับไปเริ่มต้นใหม่ |
| ใช้เมื่อไหร่ |ใช้ตอนแก้โค้ดดีไซน์ ตกแต่ง UI หรือแก้ไขข้อผิดพลาดทั่วไป |ใช้เมื่อแก้ฟังก์ชัน `initState()` หรือแก้ไขตัวแปรระดับระบบ/โครงสร้างแอปที่ Hot Reload ไม่อัปเดต |
...
```
คำตอบ: Flutter ใช้ Rendering Engine ของตัวเองชื่อว่า Skia ทำให้วาดหน้าจอได้เหมือนกันทุกแพลตฟอร์ม ส่วน React Native จะใช้ Native UI Component ของระบบปฏิบัติการ จึงมีหน้าตาและการแสดงผลตาม Android หรือ iOS มากกว่า```

**2.** อธิบายความแตกต่างระหว่าง `StatelessWidget` และ `StatefulWidget` พร้อมยกตัวอย่างการใช้งานที่เหมาะสมของแต่ละประเภท

```
คำตอบ: StatelessWidget เป็น Widget ที่ข้อมูลไม่เปลี่ยนหลังจากสร้างแล้ว เหมาะกับข้อความ รูปภาพ หรือไอคอน ส่วน StatefulWidget เป็น Widget ที่สามารถเปลี่ยนข้อมูลได้ระหว่างการใช้งาน เช่น ปุ่มนับจำนวน ตัวนับคะแนน หรือหน้าจอแชทที่ข้อมูลเปลี่ยนตลอดเวลา```

**3.** เหตุใดจึงห้าม Commit API Key ลง Git Repository? และมีวิธีจัดการ API Key อย่างปลอดภัยอย่างไรบ้าง?

```
คำตอบ: เพราะถ้า API Key หลุดไปใน Git คนอื่นสามารถนำไปใช้งานแทนเราได้ ทำให้เสียค่าใช้จ่ายหรือข้อมูลรั่วไหล วิธีที่ปลอดภัยคือเก็บไว้ในไฟล์ .env หรือ Environment Variable และเพิ่มไฟล์ดังกล่าวไว้ใน .gitignore เพื่อไม่ให้ถูกอัปโหลดขึ้น Git```

**4.** Hot Reload ทำงานอย่างไร และมีข้อจำกัดอะไรบ้าง?

```
คำตอบ: Hot Reload จะอัปเดตเฉพาะโค้ดที่แก้ไขและแสดงผลทันทีโดยไม่ต้องรันแอปใหม่ ทำให้พัฒนาได้เร็วขึ้น แต่ถ้ามีการแก้ไขส่วนของ main() การกำหนดค่าเริ่มต้น หรือ Dependency บางอย่าง จะต้องใช้ Hot Restart หรือรันแอปใหม่แทน```
...
1. ใช้เป็น AI Chatbot สำหรับตอบคำถามหรือช่วยเหลือผู้ใช้งานภายในแอป
2. ช่วยสรุปข้อความ แปลภาษา หรือสร้างเนื้อหาอัตโนมัติจากข้อมูลที่ผู้ใช้ป้อน
3. ช่วยแนะนำสินค้า ร้านอาหาร หรือคอร์สเรียนให้เหมาะกับความสนใจของผู้ใช้
```
