...
┌─────────────────────────────────────────────────────────┐
│ C:\Users\PpeeP>flutter doctor                          │
│ Doctor summary (to see all details, run doctor -v):     │
│ [✓] Flutter (Channel stable, 3.44.5, on Windows)        │
│ [✓] Windows Version (Windows 11 or higher, 25H2)        │
│ [✓] Android toolchain - Android SDK version 34.0.0      │
│ [✓] Chrome - develop for the web                        │
│ [✗] Visual Studio - develop Windows apps (Not installed)│
        ...
        └── Scaffold
            ...
            └── Padding (Body)
                └── Column
                    ├── CircleAvatar
                    │   └── Icon (Icons.person)
                    ├── Text ("ปิยะวิทย์ พลศรี")
                    ├── Text ("รหัสนักศึกษา: 67030142")
                    ├── Card
                    │   └── Padding
                    │       └── Column
                    │           ├── Row (คณะ)
                    │           │   ├── Icon (Icons.school)
                    │           │   ├── Text ("คณะ: ")
                    │           │   └── Expanded ── Text ("ครุศาสตร์อุตสาหกรรมและเทคโนโลยี")
                    │           ├── Divider
                    │           ├── Row (ความสนใจพิเศษ)
                    │           │   ├── Icon (Icons.code)
                    │           │   ├── Text ("ความสนใจพิเศษ: ")
                    │           │   └── Expanded ── Text ("Software Testing, Flutter, AI")
                    │           ├── Divider
                    │           └── Row (โปรเจกต์ที่กำลังทำ)
                    │               ├── Icon (Icons.star)
                    │               ├── Text ("โปรเจกต์ที่กำลังทำ: ")
                    │               └── Expanded ── Text ("Mongjai Project")
...
|---|---|---|
| ความเร็ว | รวดเร็วมาก (เสี้ยววินาที / < 1 วินาที) | ปานกลาง (ประมาณ 2-5 วินาที) |
| State ถูก Reset? | ไม่ถูก Reset (คงค่าของตัวแปรและสถานะเดิมไว้) | ถูก Reset ทั้งหมด (กลับสู่สถานะเริ่มต้นของโปรแกรม) |
| ใช้เมื่อไหร่ | ตกแต่ง UI, เปลี่ยนสี, ปรับขนาดฟอนต์, แก้ไขข้อความใน build | เปลี่ยนโครงสร้างโปรเจกต์, แก้ไข initState, เพิ่มคลาส/เมธอดใหม่ |

### 3.4 ผลการทดลอง Prompt Engineering

**Prompt แบบ Simple:**
```
สรุปข้อความต่อไปนี้เป็นภาษาไทยให้กระชับและสั้นที่สุด: 
การเขียนโปรแกรมไม่ใช่แค่เรื่องของตัวเลขหรือภาษาคอมพิวเตอร์ที่ดูเข้าใจยาก แต่แท้จริงแล้วมันคือ "ศิลปะของการแก้ปัญหา" ทุกครั้งที่เราเริ่มพิมพ์โค้ดบรรทัดแรก เรากำลังเปลี่ยนไอเดียในหัวให้กลายเป็นเครื่องมือที่ใช้งานได้จริงในโลกความจริง แน่นอนว่าระหว่างทางเราต้องเจอกับสิ่งที่เรียกว่า "บัก" หรือข้อผิดพลาดระบบที่ทำให้ปวดหัวอยู่เสมอ แต่ความสนุกและเสน่ห์ที่แท้จริงของมัน ก็คือวินาทีที่เราค้นพบวิธีแก้ไข แล้วเห็นแอปพลิเคชันของเราทำงานได้อย่างราบรื่นตามที่ตั้งใจไว้นั่นเอง ทุกความพยายามในการเขียนโค้ดจึงไม่ใช่แค่การสั่งงานคอมพิวเตอร์ แต่คือการฝึกคิดอย่างเป็นระบบและการเติบโตในฐานะนักพัฒนา
```
![Simple Prompt Result](screenshots/ai_studio_simple.png)

**Prompt แบบ Detailed:**
```
คุณคือผู้เชี่ยวชาญด้านการวิเคราะห์และสรุปข้อมูล (Data Analyst AI) 

หน้าที่ของคุณคือตรวจสอบข้อความที่ผู้ใช้ส่งมาด้านล่างนี้ แล้วสรุปเนื้อหาออกมาตามเงื่อนไขที่กำหนดอย่างเคร่งครัด

[ข้อความที่ต้องการให้วิเคราะห์]: 
---

[เงื่อนไขในการตอบกลับ]:
1. สรุปใจความสำคัญเป็นข้อ ๆ (Bullet points) ไม่เกิน 3 ข้อ
2. ใช้ภาษาไทยที่เข้าใจง่าย กระชับ และตรงประเด็น (ห้ามน้ำท่วมทุ่ง)
3. หากในข้อความมีความผิดพลาด หรือแนวโน้มที่เป็นอันตราย ให้เพิ่มหัวข้อ "[ข้อควรระวัง]" ไว้ที่บรรทัดสุดท้าย
4. ห้ามทักทาย ห้ามเกริ่นนำ และห้ามลงท้ายด้วยคำว่า "หวังว่าข้อมูลนี้จะมีประโยชน์" ให้แสดงผลลัพธ์ที่สรุปแล้วทันที
```
![Detailed Prompt Result](screenshots/ai_studio_detailed.png)

**ความแตกต่างของผลลัพธ์:**
```
1. Simple Prompt ให้ผลลัพธ์สั้นๆ เป็นประโยคเดียวจบโดยไม่มีรูปแบบตายตัว ควบคุมผลลัพธ์ได้ยากหากนำไปใช้จริงในระบบอัตโนมัติ
2. Detailed Prompt ได้คำตอบที่มีระเบียบตามเงื่อนไขชัดเจน (เป็นหัวข้อ Bullet point, กำหนดขอบเขตข้อมูล, กำหนดสไตล์การตอบแบบเจาะจง) เหมาะสำหรับการพัฒนาแอพพลิเคชันที่ต้องการผลลัพธ์ในรูปแบบมาตรฐานเดียวกัน (Structured Output)
```
...
**1.** Flutter แตกต่างจาก React Native อย่างไรในแง่ of Rendering Engine?

```
คำตอบ: Flutter วาด UI เองทั้งหมดโดยตรง ส่วน React Native แปลงโค้ดไปเรียกใช้ UI ดั้งเดิมของระบบ
```

**2.** อธิบายความแตกต่างระหว่าง `StatelessWidget` และ `StatefulWidget` พร้อมยกตัวอย่างการใช้งานที่เหมาะสมของแต่ละประเภท

```
คำตอบ: StatelessWidget แก้ไขหน้าจอไม่ได้ (เช่น โลโก้) ส่วน StatefulWidget เปลี่ยนแปลงหน้าจอตามข้อมูลใหม่ได้ (เช่น แชท)
```

**3.** เหตุใดจึงห้าม Commit API Key ลง Git Repository? และมีวิธีจัดการ API Key อย่างปลอดภัยอย่างไรบ้าง?

```
คำตอบ: ป้องกันคนขโมยคีย์ไปใช้และคิดเงินเรา / แก้ไขโดยการซ่อนคีย์ไว้ในไฟล์อื่นแล้วแอดลง `.gitignore`
```

**4.** Hot Reload ทำงานอย่างไร และมีข้อจำกัดอะไรบ้าง?

```
คำตอบ: อัปเดตโค้ดเข้า VM เพื่อแสดง UI ใหม่ทันทีโดยไม่รีเซ็ตสเตต / ข้อจำกัดคือใช้ไม่ได้กับการแก้ไขโครงสร้างหลัก เช่น `main()` หรือ `initState()`
```
...
```
คำตอบ: 
1. สร้าง Chatbot ตอบคำถามผู้ใช้
2. ช่วยวิเคราะห์และสรุปความคิดเห็นลูกค้า
3. ทำระบบช่วยกรอกข้อมูลอัตโนมัติจากภาพถ่าย
```
