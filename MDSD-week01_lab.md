...
└── ProfilePage
    └── Scaffold
        ├── AppBar (แถบด้านบน)
        └── SingleChildScrollView (พื้นที่เลื่อนได้)
            └── Padding (ระยะขอบ)
...
| ใช้เมื่อไหร่ | ตอนปรับแก้หน้าตา UI เช่น เปลี่ยนสี, ขนาดฟอนต์, หรือเปลี่ยนข้อความธรรมดา | ตอนเพิ่มตัวแปรใหม่, เปลี่ยนโครงสร้าง Class, หรือเพิ่งติดตั้ง Package ใหม่ |
...
```
การใช้ Prompt แบบ Simple ตัว AI จะคิดดีไซน์ให้แบบกว้างๆ (เช่น การ์ดสีพื้นฐาน) และมักจะสร้างคำอธิบายโค้ดยืดยาวติดมาด้วย ในขณะที่ Prompt แบบ Detailed จะทำให้เราควบคุมหน้าตา UI ได้แม่นยำ (เช่น การ์ดไล่สีฟ้า-ขาว, ตัวเลขอุณหภูมิขนาดใหญ่ 64) ไอคอนได้ตรงตามที่สั่งเป๊ะๆ และได้ผลลัพธ์มาเป็นโค้ดล้วนๆ ที่สะอาด พร้อมก๊อปปี้ไปใช้งานได้ทันที
```
...
```
คำตอบ:Flutter ใช้ Rendering Engine ของตัวเอง วาดกราฟิกบนหน้าจอเองทั้งหมด ทำให้แอปทำงานเร็วและหน้าตาเหมือนกันทุกแพลตฟอร์ม ส่วน React Native ต้องใช้ Bridge ในการแปลโค้ดไปเรียกใช้ UI ของระบบเครื่องนั้นๆ ซึ่งอาจทำงานช้ากว่า
```

**2.** อธิบายความแตกต่างระหว่าง `StatelessWidget` และ `StatefulWidget` พร้อมยกตัวอย่างการใช้งานที่เหมาะสมของแต่ละประเภท

```
คำตอบ: StatelessWidget คือหน้าจอแบบคงที่ สร้างครั้งเดียวจบไม่มีการอัปเดต เช่น หน้าแสดงประวัติ หรือป้ายข้อมูล ส่วน StatefulWidget คือหน้าจอที่สามารถเปลี่ยนแปลงหรืออัปเดตข้อมูลตามการกระทำของผู้ใช้ได้ตลอดเวลา
```

**3.** เหตุใดจึงห้าม Commit API Key ลง Git Repository? และมีวิธีจัดการ API Key อย่างปลอดภัยอย่างไรบ้าง?

```
คำตอบ: ห้ามอัปโหลดลง Git เด็ดขาด เพราะหากเป็น Public แฮกเกอร์สามารถใช้บอทดึงคีย์เราไปใช้จนอาจเสียเงินจำนวนมากได้ วิธีที่ปลอดภัยคือเก็บคีย์ไว้ในไฟล์ .env และตั้งค่าใน .gitignore เพื่อป้องกันไม่ให้ระบบอัปโหลดไฟล์นี้ขึ้นไปเก็บบนเซิร์ฟเวอร์
```

**4.** Hot Reload ทำงานอย่างไร และมีข้อจำกัดอะไรบ้าง?

```
คำตอบ: เป็นระบบที่ช่วยอัปเดตโค้ดหน้าจอทันที ทำให้เราเห็นผลลัพธ์การแต่ง UI ได้ไวโดยที่ข้อมูลบนจอไม่หายไป แต่มีข้อจำกัดคือจะไม่สามารถทำงานได้หากเราไปเปลี่ยนโครงสร้างหลักของโค้ด เช่น การเปลี่ยนชื่อตัวแปร หรือเปลี่ยนโครงสร้างคลาส
```
...
```
คำตอบ: 
1. ทำแชทบอทผู้ช่วยอัจฉริยะ: ฝังไว้ในแอปเพื่อตอบคำถามหรือแก้ปัญหาให้ผู้ใช้งานได้ทันที
2. สกัดข้อมูลสำคัญ: ให้ AI ช่วยดึงเฉพาะคีย์เวิร์ด วันที่ หรือเวลา จากประโยคยาวๆ เพื่อนำไปบันทึกอัตโนมัติ
3. ระบบสรุปและแปลภาษา: นำ AI มาช่วยสรุปเนื้อหาบทความยาวๆ ในแอปให้สั้นลง หรือแปลภาษาตามบริบทให้ผู้ใช้อ่านง่ายขึ้น
```
