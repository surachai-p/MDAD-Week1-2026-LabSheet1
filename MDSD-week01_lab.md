...
Flutter Version: 10.0.26200.8737
Dart Version: 3.12.2
...
|---|---|---|
| ความเร็ว |เร็วมาก (เสี้ยววินาที) |ปานกลาง (ประมาณ 1-3 วินาที) |
| State ถูก Reset? |ไม่ (ข้อมูลหรือสถานะบนหน้าจอจะยังคงอยู่เหมือนเดิม) |ใช่ (แอปจะถูกรีเซ็ตกลับไปเริ่มต้นใหม่ทั้งหมด) |
| ใช้เมื่อไหร่ |เมื่อมีการปรับแก้ UI, เปลี่ยนสี, เปลี่ยนขนาดตัวอักษร, จัด Layout หรือแก้ไขข้อความ |เมื่อมีการเพิ่มไฟล์รูปภาพ/ไฟล์ใหม่, เพิ่มแพ็กเกจ, แก้ไขฟังก์ชัน main(), หรือแก้ไขโค้ดใน initState() |

### 3.4 ผลการทดลอง Prompt Engineering

**Prompt แบบ Simple:**
```
แนวคิดของ Flutter Framework สำหรับนักศึกษาปี 2 สามารถสรุปให้เข้าใจง่ายใน 5
ประโยค ดังนี้ครับ

1.  เขียนครั้งเดียวใช้ได้ทุกที่ (Single Codebase): Flutter คือเครื่องมือจาก
    Google ที่ช่วยให้เราสร้างแอปพลิเคชันทั้ง iOS, Android
    และเว็บได้ด้วยการเขียนโค้ดเพียงชุดเดียว
2.  ทุกอย่างคือ Widget (Everything is a Widget):
    แนวคิดหลักคือการมองทุกองค์ประกอบบนหน้าจอ ตั้งแต่ปุ่ม ข้อความ
    ไปจนถึงโครงสร้างเลย์เอาต์ เป็น "ชิ้นส่วน" (Widget)
    ที่นำมาวางซ้อนหรือประกอบกันเหมือนตัวต่อ
3.  เห็นผลทันใจด้วย Hot Reload:
    จุดเด่นที่ช่วยให้พัฒนาได้ไวคือฟีเจอร์ที่ทำให้เราเห็นการเปลี่ยนแปลงบนหน้าจอแอปได้ทันทีหลังแก้ไขโค้ด
    โดยไม่ต้องเสียเวลารันใหม่ตั้งแต่ต้น
4.  ใช้ภาษา Dart เป็นหัวใจ: พัฒนาด้วยภาษา Dart ซึ่งมีโครงสร้างแบบ
    Object-Oriented (OOP) ที่เข้าใจง่ายสำหรับคนที่เคยผ่าน Java หรือ C++ มาก่อน
    และออกแบบมาเพื่อสร้าง UI โดยเฉพาะ
5.  วาดหน้าจอเองเพื่อประสิทธิภาพสูง: ต่างจาก Framework อื่นตรงที่ Flutter
    มีเอนจินกราฟิกของตัวเองเพื่อวาด UI ขึ้นมาใหม่ทั้งหมด
    ทำให้แอปทำงานได้ลื่นไหลและมีหน้าตาเหมือนกันเป๊ะในทุกอุปกรณ์
```
...
```
คำตอบ: Flutter มี Rendering Engine เป็นของตัวเอง (ใช้ Skia หรือ Impeller) ซึ่งจะวาดพิกเซล UI ทุกชิ้นขึ้นมาเองบนหน้าจอ ทำให้แอปหน้าตาเหมือนกัน 100% ในทุกแพลตฟอร์ม ในขณะที่ React Native ใช้กลไก Bridge เพื่อแปลงโค้ดไปเรียกใช้ Native UI Components ของแต่ละระบบปฏิบัติการ (iOS/Android) ซึ่งอาจทำให้หน้าตาแอปแสดงผลต่างกันเล็กน้อยตาม OS นั้นๆ
```

**2.** อธิบายความแตกต่างระหว่าง `StatelessWidget` และ `StatefulWidget` พร้อมยกตัวอย่างการใช้งานที่เหมาะสมของแต่ละประเภท

```
คำตอบ:- StatelessWidget: คือหน้าจอหรือชิ้นส่วนที่ "ไม่มีสถานะ" เมื่อวาดเสร็จแล้วหน้าตาจะคงที่ไม่มีการเปลี่ยนแปลง เหมาะกับ UI นิ่งๆ เช่น หน้าแสดง Profile Card, ไอคอน, โลโก้แอป หรือข้อความอธิบาย
- StatefulWidget: คือหน้าจอที่ "มีสถานะ" สามารถวาดตัวเองใหม่เพื่ออัปเดตหน้าตาได้เมื่อมีข้อมูลเปลี่ยนแปลงระหว่างใช้งาน เหมาะกับ UI ที่มีการโต้ตอบ เช่น หน้าแชท AI ที่มีข้อความเพิ่มขึ้นเรื่อยๆ, ช่องกรอกฟอร์ม, ปุ่มกดนับคะแนน หรือหน้าจอที่มี Animation
```

**3.** เหตุใดจึงห้าม Commit API Key ลง Git Repository? และมีวิธีจัดการ API Key อย่างปลอดภัยอย่างไรบ้าง?

```
คำตอบ:การนำ API Key ไป Commit ลง Public Repository จะทำให้ผู้ไม่หวังดีสามารถขโมย Key ของเราไปใช้งานได้ ซึ่งอาจนำไปสู่การถูกเรียกเก็บเงินมหาศาล หรือถูกบล็อกบัญชีผู้พัฒนา วิธีจัดการที่ปลอดภัยคือ การเก็บ API Key ไว้ในไฟล์คอนฟิก (เช่น ไฟล์ .env หรือ api_config.dart) และต้องนำชื่อไฟล์นั้นไปใส่ไว้ในไฟล์ .gitignore เสมอ เพื่อป้องกันไม่ให้ Git อัปโหลดไฟล์ดังกล่าวขึ้นไปยัง Server
```

**4.** Hot Reload ทำงานอย่างไร และมีข้อจำกัดอะไรบ้าง?

```
คำตอบ: Hot Reload ทำงานโดยการฉีดโค้ด (Inject) ที่ถูกแก้ไขใหม่เข้าไปใน Dart Virtual Machine (VM) ที่กำลังรันอยู่ ทำให้ UI บนหน้าจออัปเดตทันทีโดยไม่ต้องเริ่มแอปใหม่ ข้อจำกัดคือ จะไม่ทำงานหากมีการแก้ไขโค้ดโครงสร้างหลัก เช่น ฟังก์ชัน main(), การเพิ่ม/ลดแพ็กเกจใหม่, การเปลี่ยนตัวแปร Global หรือการแก้ไขข้อมูลในฟังก์ชัน initState() ซึ่งกรณีเหล่านี้จะต้องใช้ Hot Restart แทน
```
...
```
คำตอบ: 
1. นำมาทำระบบ AI Tutor หรือผู้ช่วยส่วนตัวในแอปการศึกษา เพื่อคอยอธิบายคำสั่งยากๆ หรือช่วยวิเคราะห์และแก้ Error ให้ผู้เรียนเป็นภาษาไทยแบบเข้าใจง่าย
2. ใช้สร้างสถานการณ์จำลอง (Scenario Generation) ในแอปพลิเคชันหรือเกมแบบไดนามิก เช่น การสุ่มสร้างโจทย์ปัญหาความขัดแย้งของข้อมูล (Merge Conflict) ให้ผู้ใช้ฝึกแก้ปัญหาในแต่ละด่านแบบไม่ซ้ำกัน
3. ใช้ทำระบบวิเคราะห์ประวัติการพิมพ์โต้ตอบของผู้ใช้ เพื่อสรุปหา Pain point ว่าผู้ใช้งานมักจะติดปัญหาหรือสับสนในหัวข้อไหนมากที่สุด นำไปสู่การปรับปรุงเนื้อหาแอปพลิเคชันให้ดีขึ้น
```
