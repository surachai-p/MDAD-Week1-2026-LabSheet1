...
**Prompt แบบ Simple:**
```
เปรียบเทียบความต่างระหว่าง iPad Air กับ iPad Pro แบบสั้นๆ
```

**Prompt แบบ Detailed:**
```
ช่วยจัดทริปเที่ยว พัทยา แบบ 2 วัน 1 คืนเน้นทริปพักผ่อนชอบคาเฟ่ริมทะเลถ่ายรูปสวยๆ
```

**ความแตกต่างของผลลัพธ์:**
```
การใช้คำสั่งที่สั้นและกระชับ (Simple) ทำให้ AI เลือกข้อมูลเหลือเพียงสาระสำคัญที่จำเป็น ตอบกลับมาเป็นหัวข้อย่อยที่อ่านง่ายและได้ข้อสรุปในทันที เหมาะสำหรับการหาข้อมูลพื้นฐานเพื่อตัดสินใจอย่างรวดเร็ว
(Detailed) AI จะเปลี่ยนบทบาทเป็นผู้เชี่ยวชาญที่เข้ามาช่วยวางแผนอย่างเป็นระบบ มีการคิดคอนเซปต์ แนะนำสถานที่ที่ตรงกับไลฟ์สไตล์ และจัดหมวดหมู่ข้อมูลให้อย่างสมบูรณ์แบบพร้อมใช้งาน
```
...
```
คำตอบ:  StatelessWidget = ข้อมูลคงที่ ไม่เปลี่ยนระหว่างใช้งาน
 StatefulWidget = ข้อมูลเปลี่ยนได้ และหน้าจอจะอัปเดตตามข้อมูลที่เปลี่ยน
```

**3.** เหตุใดจึงห้าม Commit API Key ลง Git Repository? และมีวิธีจัดการ API Key อย่างปลอดภัยอย่างไรบ้าง?

```
คำตอบ: เพราะหาก API Key หลุดไปอยู่ใน Git Repository โดยเฉพาะแบบสาธารณะ ผู้อื่นสามารถนำไปใช้งานแทนเราได้ ทำให้ข้อมูลรั่วไหลหรือเกิดค่าใช้จ่ายโดยไม่ได้รับอนุญาต วิธีที่ปลอดภัยคือเก็บ API Key ไว้ในไฟล์ .env หรือ Environment Variables, เพิ่มไฟล์ที่มี API Key ลงใน .gitignore และกำหนดสิทธิ์การใช้งานของ API Key ให้เหมาะสม
```

**4.** Hot Reload ทำงานอย่างไร และมีข้อจำกัดอะไรบ้าง?

```
คำตอบ: Hot Reload จะอัปเดตเฉพาะโค้ดที่แก้ไขและแสดงผลบนแอปทันที โดยไม่ต้องปิดและเปิดแอปใหม่ ทำให้พัฒนาได้รวดเร็วและยังคงสถานะ (State) ของแอปไว้ ข้อจำกัดคือไม่รองรับการเปลี่ยนแปลงบางอย่าง เช่น การแก้ไขโค้ดเริ่มต้นของแอป (main()), การเปลี่ยนแปลง Native Code หรือการเพิ่ม Dependency ใหม่ ซึ่งต้องใช้ Hot Restart หรือรันแอปใหม่
```
...
```
คำตอบ: 
1. AI Chatbot – ตอบคำถามหรือให้คำแนะนำแก่ผู้ใช้งานภายในแอป
2. สรุปหรือสร้างเนื้อหาอัตโนมัติ – เช่น สรุปบทความ เขียนข้อความ หรือสร้างคำอธิบายสินค้า
3. ผู้ช่วยแปลภาษา – แปลข้อความระหว่างหลายภาษาแบบเรียลไทม์ ช่วยให้ผู้ใช้สื่อสารได้สะดวกขึ้น
```
