...
|---|---|---|
| ความเร็ว | 371ms | 398ms |
| State ถูก Reset? | ไม่รีเซ็ต | รีเซ็ต |
| ใช้เมื่อไหร่ | แก้เล็กๆน้อยๆ | แก้โครงสร้างหรืออะไรที่ใหญ่ๆ |
...
```
คำตอบ: - Flutterจะวาดuiเองทั้งหมดผ่านEngineของตัวเอง

- React Nativeจะใช้jsสั่งงานผ่านBridge/JSIไปเรียกNative Component ของOS มาเรนเดอร์แทนเช่น UIView
```

**2.** อธิบายความแตกต่างระหว่าง `StatelessWidget` และ `StatefulWidget` พร้อมยกตัวอย่างการใช้งานที่เหมาะสมของแต่ละประเภท

```
คำตอบ: - StatelessWidget — สร้างครั้งเดียวแล้วหน้าตาไม่เปลี่ยน ใช้กับข้อความ ไอคอน หรือป้ายแสดงข้อมูลทั่วไป

- StatefulWidget — เก็บค่าที่เปลี่ยนแปลงได้ระหว่างใช้งาน เวลาค่าเปลี่ยนต้องสั่งให้วาดหน้าจอใหม่ ใช้กับตัวนับ ฟอร์มกรอกข้อมูล หรือช่องกาเครื่องหมาย
```

**3.** เหตุใดจึงห้าม Commit API Key ลง Git Repository? และมีวิธีจัดการ API Key อย่างปลอดภัยอย่างไรบ้าง?

```
คำตอบ: - ถ้าอัปโหลดขึ้นที่สาธารณะเช่นGitHub แบบเปิดจะมีคนหรือบอทคอยสแกนหาหรือจะโดนแฮกก


- เก็บคีย์ไว้ในไฟล์แยกต่างหาก เช่น .env แล้วใส่ชื่อไฟล์นั้นเข้าไปใน .gitignore ไม่ให้กิตติดตาม
```

**4.** Hot Reload ทำงานอย่างไร และมีข้อจำกัดอะไรบ้าง?

```
คำตอบ: - เวลาแก้โค้ดแล้วบันทึก ระบบจะส่งเฉพาะส่วนที่แก้เข้าไปในแอปที่รันอยู่ แล้ววาดหน้าจอใหม่ทันที โดยไม่ปิดแอปและไม่เสียข้อมูลที่กรอกไว้ เห็นผลไวมาก

- แก้ค่าตั้งต้นตอนเปิดแอปครั้งแรกแล้วไม่เห็นผล ต้องเปิดแอปใหม่
...
```
คำตอบ: 
1. ช่วยเขียนโค้ดและแก้บั๊ก
ให้ AI ช่วยแนะนำโค้ด Dart/Flutter หรือช่วยหาจุดที่ทำให้เกิดข้อผิดพลาด ทำให้เขียนโปรแกรมได้เร็วขึ้นและลดเวลาไล่หาปัญหา
2. สร้างเนื้อหาหรือข้อความในแอปแบบอัตโนมัติ
เช่น แอปแชทบอทตอบคำถามลูกค้า แอปแนะนำสินค้า หรือแอปที่ต้องสรุปข้อความยาวๆ ให้สั้นลง ใช้ AI ช่วยประมวลผลภาษาแทนคน
3. วิเคราะห์ข้อมูลและให้คำแนะนำผู้ใช้
เช่น แอปสุขภาพที่วิเคราะห์พฤติกรรมแล้วแนะนำการออกกำลังกาย หรือแอปการเงินที่ช่วยวิเคราะห์การใช้จ่ายแล้วให้คำแนะนำเฉพาะบุคคล
```
